...
SRC_FILES := $(wildcard $(SRC_DIR)/*.cpp) $(wildcard $(SRC_DIR)/*/*.cpp) $(wildcard $(SRC_DIR)/*/*/*.cpp) $(wildcard $(GAME_DIR)/*.cpp) $(wildcard $(NET_DIR)/*.cpp)
COMMON_FILES := $(wildcard $(COMMON_DIR)/*.cpp)
# NET_FILES := $(wildcard $(NET_DIR)/*.cpp)
GAME_FILES := $(wildcard $(GAME_DIR)/*.cpp)

OBJ_FILES := $(patsubst $(SRC_DIR)/%.cpp,$(SRC_DIR)/%.o,$(SRC_FILES)) $(patsubst $(COMMON_DIR)/%.cpp,$(COMMON_DIR)/%.o,$(COMMON_FILES)) $(patsubst $(GAME_DIR)/%.cpp,$(GAME_DIR)/%.o,$(GAME_FILE)) $(patsubst $(NET_DIR)/%.cpp,$(NET_DIR)/%.o,$(NET_FILES))
